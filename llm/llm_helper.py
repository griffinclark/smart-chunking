...
def fetch_documents_from_cassandra( session, keyspace, limit):
    ...
    select_stmt = f"SELECT text FROM {keyspace}.text_embeddings LIMIT %s;"
...
def get_response_from_query(query, k, user_role):
    ...
    docs = fetch_documents_from_cassandra(astraSession, ASTRA_DB_KEYSPACE, k)
